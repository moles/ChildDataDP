...
Ext.ux.MenuPanel = function (config) {
    Ext.ux.MenuPanel.superclass.constructor.call(this,
    {
        ...
        title: config.title,
        split: true,
        width: config.width ? config.width : 170,
        maxSize: 400,
        collapsible: true,
        margins: '0 0 5 5',
        cmargins: '0 5 5 5',
	...
	initComponent: function () {
        ...
        Ext.apply(this,
        {
            ...
            items: new Ext.Panel(
            {
                layout: 'border',
                tbar: this.topPanel,
                items: [(this.menuPanel != null ? this.menuPanel : new Ext.Panel(
                {
                    hidden: true
...
initMenuPanel: function (config) {
    ...
    this.menuPanel.on('menuitemselect', function (node) {
        componentViewer.updateTitle();
		componentViewer.updateBottomPanel(); //OCFS actualiza el status para los plugins que no son default
        componentViewer.updateTopPanel(); //OCFS actualiza el topPanel para los plugins que no son default
        componentViewer.componentHolder.loadComponent(node);
    });    
},
updateLink: function(fn)
    ...
    updateMenu: function()
    {
        if (this.menuPanel) 
        {
            ...
            {            
                //node.eachChild(componentViewer.validateMenuLink, componentViewer);
                node.eachChild(function(node)
                {
                    componentViewer.validateMenuLink(node, componentViewer);
                })
            });
        }
    },
