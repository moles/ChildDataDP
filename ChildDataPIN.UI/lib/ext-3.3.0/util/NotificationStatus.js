...
Ext.ux.ValidationStatus = Ext.extend(Ext.Component, {
     ...
     */
    //showText : 'The form has notifications (click for details...)',
	showText : '',
    /**
     * @cfg {String} showText
     ...
     */
    hideText : 'Click again to hide the notification list',
    /**
    ...
    init : function(sb){
        sb.on('render', function(){
			...
			this.notifications = new Ext.util.MixedCollection();
            //this.listAlign = (sb.statusAlign=='right' ? 'br-tr?' : 'bl-tl?');
			this.listAlign = 'bl-tl?';
			
			this.notificationsButton = new Ext.Button({
				text:'Notifications (0)',
				cls:'x-button-text-icon',
				tooltip: 'Notifications',
				icon:'../resources/images/application/information.png',
			});          
			this.notificationsButton.on('click', this.onNotificationsClick, this);

			/*this.errorsButton = new Ext.Button({
				text:'0',
				cls:'x-button-text-icon',
				tooltip:'Errors in form',
				icon:'../resources/images/application/error.png',
			});       
			this.errorsButton.on('click', this.onErrorsClick, this);
			
			this.statusBar.insertButton(1, '-');
			this.statusBar.insert(2, this.errorsButton);*/
			this.statusBar.insert(1, '-');
			this.statusBar.insert(2, this.notificationsButton);
			
			if(this.form){				
				this.form = Ext.getCmp(this.form);
				
				this.form.on('notificationchange', this.onNotificationChange, this);
    ...
    stopMonitoring : function(){
		...
		this.form.un('notificationchange', this.onNotificationChange, this);
    },
    ...
    onFieldValidation : function(f, msg){
        ...
        if(this.errors.getCount() > 0){
			this.statusBar.setStatus({text:"Errors (" + this.errors.getCount().toString() + ")", iconCls:this.errorIconCls});
			...
			this.statusBar.setStatus({text:"OK", iconCls:this.validIconCls});
        }
    },
    onNotificationChange: function(id, msg)
	{
		if(!this.monitor){
            return false;
        }
        if(msg){
            this.notifications.add(id, {id: id, msg:msg});
        }else{
            this.notifications.removeKey(id);
        }
        this.updateNotificationList();
        if(this.notifications.getCount() > 0){
            if(this.statusBar.getText() != this.showText){
                //this.statusBar.setStatus({text:this.showText, iconCls:this.errorIconCls});
            }
        }else{
    ...
    updateErrorList : function(){
		...
		//this.errorsButton.setText(this.errors.getCount().toString());
    },
    updateNotificationList : function(){
        if(this.notifications.getCount() > 0){
	        var msg = '<ul>';
	        this.notifications.each(function(notification){
	            msg += ('<li id="x-err-'+ notification.id +'"><a href="#">' + notification.msg + '</a></li>');
	        }, this);
	        this.getMsgEl().update(msg+'</ul>');
        }else{
            this.getMsgEl().update('');
		...
		this.notificationsButton.setText('Notifications (' + this.notifications.getCount().toString() +')');
    },
    // private
    getMsgEl : function(){
        if(!this.msgEl){
    ...
    onNotificationsClick : function(){
        if(this.getMsgEl().isVisible()){
            this.hideErrors();
        }else if(this.notifications.getCount() > 0){
